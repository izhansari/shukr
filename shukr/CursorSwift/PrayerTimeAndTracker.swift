...
//                }
    ...
    private func handleDragEndNew(translation: CGSize, isDraggingVertically: Bool?) {
        ...
        // positive
        let satisfiedDragDown = translation.height > threshold
        let satisfiedDragRight = translation.width > threshold
        
        // negative
        let satisfiedDragUp = translation.height < -threshold
        let satisfiedDragLeft = translation.width < -threshold
                

        withAnimation(.spring(duration: 0.5)) {
            dragOffsetNew = .zero
            isNumberEntryFocused = false
            self.isDraggingVertically = nil
...
//                switch showSalahTab {
//                case true:
//                    if satisfiedDragLeft { sharedState.showSalahTab = false ; triggerSomeVibration(type: .light) }
//                case false:
//                    if satisfiedDragRight { sharedState.showSalahTab = true ; triggerSomeVibration(type: .light) }
//                }
//            }
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            if sharedState.navPosition == .right{
                Text("Right")
            }
...
//                Text("Left")
                ...
                .opacity(dragOffsetNew.width < 0 ? 1 - abs(Double(dragOffsetNew.width / 70)) : 1)
                .transition(.move(edge: .leading).combined(with: .opacity))
            }
...
//                                sharedState.showSalahTab = true
                            ...
                            .foregroundColor( sharedState.navPosition == .left /*showSalahTab*/ ? .green : .gray)
                            .frame(width: 100)
                        }
...
//                                sharedState.showSalahTab = true
                            ...
                            .foregroundColor( sharedState.navPosition == .bottom /*showSalahTab*/ ? .green : .gray)
                            .frame(width: 100)
                        }
...
//                        Text(toggledText ? "\(prayerObject.englishScore ?? "uhoh") (\(prayerObject.numberScore ?? 00)%": "@ \(shortTimePM(completedTime))")
                        Text(toggledText ? prayerObject.numberScore != 0 ? "\((prayerObject.numberScore ?? 00)*100, specifier: "%.0f")% left" : "Kaza" : "@ \(shortTimePM(completedTime))")
                            .font(timeFontSize)
                            .foregroundColor(.secondary.opacity(statusBasedOpacity))
                    }
                } else {
