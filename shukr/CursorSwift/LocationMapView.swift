...
//            if !viewModel.showPrayers {
                VStack {
                    Spacer()
                    CircleWithArrowOverlay3(degrees: viewModel.qiblaDirection)
                        .padding()
                    Spacer()
                }
...
struct CircleWithArrowOverlay3: View {
    var degrees: Double
...
/*
 ...
 struct MapView: UIViewRepresentable {
     ...
     class Coordinator: NSObject, MKMapViewDelegate {
         ...
         // Monitor region changes to update annotations
         func mapView(_ mapView: MKMapView, regionDidChangeAnimated animated: Bool) {
             updateAnnotations(for: mapView.visibleMapRect)
         }
                         ...
                         Button(action: {
                             if let mapView = mapViewRef {
                                 if let userLocation = mapView.userLocation.location?.coordinate {
                                     let currentSpan = mapView.region.span
                                     let maxSpan = MKCoordinateSpan(latitudeDelta: 0.15, longitudeDelta: 0.15)
                                     let closeSpan = MKCoordinateSpan(latitudeDelta: 0.02, longitudeDelta: 0.02)
